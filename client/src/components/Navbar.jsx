    ...
    if (role === 'admin') {
      return (
        <>
          ...
          <AvatarMenu initials={initials} name={dbUser.name} onLogout={handleLogout} />
        </>
      );
    }
    if (role === 'mentor') {
    ...
    <header className="sticky top-0 z-50 glass">
      <nav className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link to="/home" className="flex items-center gap-2">
          ...
          <span className="text-2xl font-black tracking-tight text-blue-600">EduHub</span>
        </Link>

        <div className="hidden items-center gap-8 md:flex">
          {onHome ? (
            <>
              <a className="text-sm font-semibold transition-colors hover:text-blue-500" href="#features">Features</a>
              <a className="text-sm font-semibold transition-colors hover:text-blue-500" href="#how-it-works">How it works</a>
              <a className="text-sm font-semibold transition-colors hover:text-blue-500" href="#colleges">Colleges</a>
              <a className="text-sm font-semibold transition-colors hover:text-blue-500" href="#testimonials">Testimonials</a>
            </>
          ) : (
            <NavLink to="/home" className="text-sm font-semibold transition-colors hover:text-blue-600">
              Home
            </NavLink>
          )}
        </div>
...
function AvatarMenu({ initials, name, onLogout }) {
  return (
    <div className="relative group">
      ...
      <div className="absolute right-0 top-full mt-2 w-44 rounded-xl bg-white shadow-xl ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-700 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-150 z-50">
