...
import { Badge, PageHeader, TableWrap, TableSearch, EmptyState, BtnPrimary, BtnSecondary, FormGroup, FormInput, FormSelect, tw } from "../../components/admin/adminUtils";

const MOCK_STUDENTS = [
  { _id: "1", name: "Ahmed Samy",    email: "ahmed@edu.com",  course: "Data Structures", enrolledAt: "2025-02-01" },
  { _id: "2", name: "Nour Tarek",    email: "nour@edu.com",   course: "Web Dev",         enrolledAt: "2025-02-10" },
  { _id: "3", name: "Omar Khalid",   email: "omar@edu.com",   course: "Algorithms",      enrolledAt: "2025-02-15" },
  { _id: "4", name: "Layla Hassan",  email: "layla@edu.com",  course: "Web Dev",         enrolledAt: "2025-03-01" },
  { _id: "5", name: "Karim Ali",     email: "karim@edu.com",  course: "Databases",       enrolledAt: "2025-03-05" },
  { _id: "6", name: "Sara Mostafa",  email: "sara@edu.com",   course: "Programming",     enrolledAt: "2025-03-08" },
];
...
function Students() {
  ...
  const handleAdd = async () => {
    ...
    setSaving(true);
    // TODO: POST /api/mentor/enroll  { email: form.studentEmail, course: form.course }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <BtnPrimary onClick={() => { setForm(EMPTY_FORM); setModal(true); }}>
      ...
      >
        {filtered.length === 0 ? (
          <EmptyState icon="🎓" title="No students found" description="Try a different search or add a new student." />
        ) : (
              ...
              {filtered.map((s, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className={tw.td} style={{ color: "var(--text-secondary)", borderColor: "var(--border)" }}>{s.email}</td>
                    <td className={tw.td} style={{ borderColor: "var(--border)" }}><Badge variant="blue">{s.course}</Badge></td>
                    <td className={tw.td} style={{ color: "var(--text-muted)", borderColor: "var(--border)" }}>{s.enrolledAt}</td>
                    <td className={tw.td} style={{ borderColor: "var(--border)" }}>
                      <div className="flex justify-end">
                        <button
                          ...
                          onClick={() => setStudents((prev) => prev.filter((x) => x._id !== s._id))}
                        >
        ...
        >
          ...
          >
            <div className="flex items-center justify-between px-6 pt-5 pb-4" style={{ borderBottom: "1px solid var(--border)" }}>
              <h3 className="text-[16px] font-bold" style={{ color: "var(--text-primary)" }}>Add Student to Course</h3>
