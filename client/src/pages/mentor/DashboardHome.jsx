...
import { Badge, PageHeader, TableWrap, EmptyState, BtnPrimary, BtnDanger, tw } from "../../components/admin/adminUtils";
...
const MOCK_PENDING = [
  { _id: "1", title: "Intro to Linked Lists",       uploader: "Ahmed Samy",    course: "Data Structures", uploadedAt: "2025-03-10T10:00:00Z" },
  { _id: "2", title: "CSS Flexbox Deep Dive",       uploader: "Nour Tarek",    course: "Web Dev",         uploadedAt: "2025-03-11T08:30:00Z" },
  { _id: "3", title: "Binary Trees Explained",      uploader: "Omar Khalid",   course: "Algorithms",      uploadedAt: "2025-03-11T14:00:00Z" },
  { _id: "4", title: "React Hooks Tutorial",        uploader: "Layla Hassan",  course: "Web Dev",         uploadedAt: "2025-03-12T09:15:00Z" },
  { _id: "5", title: "SQL Joins Crash Course",      uploader: "Karim Ali",     course: "Databases",       uploadedAt: "2025-03-12T11:45:00Z" },
];
...
function DashboardHome() {
  ...
  const submitFeedback = () => {
    // TODO: call real API  PUT /api/videos/:id/review  { status, feedback }
    setPending((prev) => prev.filter((v) => v._id !== feedbackId));
    setStats((prev) => ({
      ...prev,
      pendingReviews: prev.pendingReviews - 1,
      ...(actionType === "approve" ? { approved: prev.approved + 1 } : { rejected: prev.rejected + 1 }),
    }));
    setFeedbackId(null);
  };

  const statCards = [
    { title: "Pending Reviews", value: loading ? "…" : String(stats?.pendingReviews ?? "—"), icon: "🎬", iconColor: "amber",  delta: "awaiting action", deltaType: "up"   },
    { title: "Approved Videos", value: loading ? "…" : String(stats?.approved       ?? "—"), icon: "✅", iconColor: "green",  delta: "total approved",  deltaType: "up"   },
    { title: "Rejected Videos", value: loading ? "…" : String(stats?.rejected       ?? "—"), icon: "❌", iconColor: "red",    delta: "total rejected",  deltaType: "down" },
    { title: "My Students",     value: loading ? "…" : String(stats?.students       ?? "—"), icon: "🎓", iconColor: "blue",   delta: "enrolled",        deltaType: "up"   },
  ];
          ...
          <table className="w-full">
            <thead>
              <tr style={{ background: "var(--bg-card)" }}>
                {["Video Title", "Uploader", "Course", "Uploaded", "Actions"].map((h) => (
            ...
            <tbody>
              {pending.map((v) => (
                <tr key={v._id} className={tw.trHover} style={{ borderBottom: "1px solid var(--border)" }}
          ...
          >
            ...
            <div className="p-6 space-y-4">
              <p className="text-[13px]" style={{ color: "var(--text-secondary)" }}>
                Add feedback for the student <span className="font-semibold" style={{ color: "var(--text-primary)" }}>(optional)</span>:
              </p>
              <textarea
                rows={4}
