...
export default function EmailVerification() {
  ...
  useEffect(() => {
    ...
    if (currentUser.emailVerified) { navigate('/home'); return; }
    setUser(currentUser);
  }, [navigate]);
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      setStatus('error');
      // Show the real Firebase error so we know what went wrong
      if (err.code === 'auth/too-many-requests') {
        ...
        setErrorMsg(err.message || 'Failed to send email. Please try again later.');
      }
    }
  };
      ...
      <div className="w-full max-w-md">
        ...
        <div className="rounded-2xl bg-white p-10 shadow-xl shadow-slate-900/10 ring-1 ring-slate-200 dark:bg-slate-900/40 dark:ring-slate-700">
          ...
          <div className="mt-8 space-y-3">
            <button onClick={handleCheckNow} disabled={checkPulse}
