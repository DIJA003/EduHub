...
const EMPTY = { title: '', course: '', type: 'PDF', size: '', uploader: '', status: 'Draft' };
...
function MaterialsManagement() {
  ...
  const openEdit   = (m) => { setForm(m); setEditId(m._id); setModal(true); };
  ...
  const TD = ({ children, style }) => (
    <td className={tw.td} style={{ borderBottomColor: 'var(--border-light)', color: 'var(--text-primary)', ...style }}>{children}</td>
  );
    ...
    <div>
      ...
      {error && (
        <div className="mb-4 rounded-lg px-4 py-3 text-sm" style={{ background: 'var(--danger-bg)', color: 'var(--danger)' }}>
          {error} — <button className="underline" onClick={loadMaterials}>Retry</button>
        </div>
      )}
      ...
      >
        {loading ? (
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              {filtered.map((m) => (
                <tr key={m._id} className={tw.trHover}
                ...
                >
                  ...
                  <TD><span className="font-mono text-[12px]" style={{ color: 'var(--text-muted)' }}>{formatDate(m)}</span></TD>
                  ...
                  <TD>
                    <div className="flex items-center gap-2 justify-end">
                      ...
                      <BtnDanger    className={tw.btnSm} onClick={() => handleDelete(m._id)}>Delete</BtnDanger>
                    </div>
                  </TD>
                </tr>
              ))}
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <FormGroup label="File Size (optional)">
                <FormInput value={form.size} onChange={(e) => set('size', e.target.value)} placeholder="e.g. 2.4 MB" />
              </FormGroup>
