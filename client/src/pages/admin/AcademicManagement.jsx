...
function AcademicManagement() {
  ...
  const openEdit   = (col) => { setForm(col);   setEditId(col._id); setModal(true); };
    ...
    <div>
      ...
      {error && (
        <div className="mb-4 rounded-lg px-4 py-3 text-sm" style={{ background: 'var(--danger-bg)', color: 'var(--danger)' }}>
          {error} — <button className="underline" onClick={loadColleges}>Retry</button>
        </div>
      )}
            ...
            <thead style={{ background: 'var(--bg-card)' }}>
              <tr>
                {['College Name', 'Duration', 'Semesters/Year', 'Programs', 'Status', 'Actions'].map((h) => (
                  <th key={h} className={tw.th} style={{ color: 'var(--text-muted)', borderBottomColor: 'var(--border)' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.map((col) => (
                <tr key={col._id} className={tw.trHover}
                ...
                >
                  ...
                  <td className={tw.td} style={{ borderBottomColor: 'var(--border-light)' }}>
                    <div className="flex items-center gap-2 justify-end">
                      ...
                      <BtnDanger    className={tw.btnSm} onClick={() => handleDelete(col._id)}>Delete</BtnDanger>
                    </div>
                  </td>
                </tr>
              ))}
