...
export default function StudentDashboard() {
  ...
  const { years, enrollCourse, undoEnrollment, currentYearId, loading: coursesLoading } = useCourses();
  ...
  const sidebar   = "bg-slate-950 border-slate-800";
  const cardBg    = "bg-slate-800/60 border-slate-700";
  const courseTxt = "text-slate-300";
  const hoverLink = "hover:bg-slate-800 hover:text-white";
              ...
              <li key={link.id}>
                <button
                  ...
                  className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm transition ${
                    activeLink === link.id ? "bg-slate-700 text-white" : `text-slate-300 ${hoverLink}`
                  }`}
                >
                  {link.label}
                </button>
                  ...
                  <div className="space-y-3">
                    {activityLog.slice(0, 10).map((entry) => (
                      <div key={entry.id} className="flex items-start gap-4 rounded-xl border border-slate-700 bg-slate-800/50 px-4 py-3">
                        <span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-slate-700 text-base">{entry.icon}</span>
            ...
            <section>
              ...
              {coursesLoading ? (
                <div className="grid gap-4 sm:grid-cols-2">
                  {[1,2].map((i) => <div key={i} className="h-28 animate-pulse rounded-xl border border-slate-700 bg-slate-800/50" />)}
                </div>
              ) : (
                <div className="grid gap-4 sm:grid-cols-2">
                  {enrolledCourses.length === 0 ? (
                    <div className="col-span-2 rounded-xl border border-slate-700 bg-slate-800/50 p-6">
                      <p className="text-slate-400">No courses enrolled yet. Enroll from the Available section below.</p>
                    </div>
                  ) : (
                    enrolledCourses.map((course) => (
                      <div key={course.id} className={`rounded-xl border ${cardBg} p-5`}>
                        ...
                        <div className="mt-2 flex items-center justify-between gap-2">
                          <p className="text-xs text-slate-400">
                            Next: <span className="text-slate-300">{course.nextItem}</span>
                          </p>
                          <button
                            type="button"
              ...
              {coursesLoading ? (
                <div className="mt-3 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {[1,2,3].map((i) => <div key={i} className="h-36 animate-pulse rounded-xl border border-slate-700 bg-slate-800/50" />)}
                </div>
              ) : (
                <div className="mt-3 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {availableCourses.length === 0 ? (
                    <div className="col-span-3 rounded-xl border border-slate-700 bg-slate-800/50 p-6">
                      <p className="text-slate-400">
                        No available courses found. An admin needs to publish courses for this year, or you may have reached your credit limit.
                      </p>
                    </div>
                  ) : (
                    availableCourses.map((course) => (
                      <div key={course.id} className={`rounded-xl border ${cardBg} p-5`}>
                        <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-500">{course.type || "Course"}</p>
                        ...
                        <p className="mt-1 text-xs text-slate-400">{course.credits} credits</p>
                        <button
                          type="button"
                    ...
                    <div key={m.id} className="flex items-start gap-4 rounded-xl border border-slate-700 bg-slate-800/50 px-4 py-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className={`text-sm ${courseTxt}`}>
                          ...
                          — <span className={STATUS_COLOR[m.status] || "text-slate-400"}>{STATUS_LABEL[m.status] || m.status}</span>
                        </p>
                        ...
                        <button type="button" className="mt-2 text-xs text-rose-400 hover:text-rose-300" onClick={() => removeMaterial(m.id)}>Remove</button>
                      </div>
                    </div>
                  ))
                )}
