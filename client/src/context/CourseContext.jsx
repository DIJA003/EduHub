  ...
  years: {
    ...
    4: {
      meta: {
        ...
        description: "Capstone, research, and industry placement.",
        status: "Locked",
        earnedCredits: 0,
        totalCredits: 42,
        unlocked: false,
...
function mergeStoredState(base, stored) {
  ...
  const merged = {
    ...base,
    lastCompletedCourse: stored.lastCompletedCourse ?? base.lastCompletedCourse,
  };
    ...
    yearsOut = {
      ...
      [nextId]: {
        ...
        meta: {
          ...
          unlocked: true,
          status:
            nextYear.meta.status === "Locked"
              ? "In Progress"
              : nextYear.meta.status,
        },
      },
    };
  }
  ...
  useEffect(() => {
    ...
    const syncFromBackend = async () => {
      try {
          ...
          enrollments.forEach((e) => {
            ...
            const updatedCourse = {
              ...
              sectionsCompleted: Math.max(
                e.sectionsCompleted || 0,
                existing?.sectionsCompleted || 0,
              ),
            ...
            const enrolled = existing
              ? year.enrolled.map((c) =>
                  c.id === e.id ? { ...c, ...updatedCourse } : c,
                )
              : [...year.enrolled, updatedCourse];

            years[yid] = withYearEarnedCredits({
              ...year,
              enrolled,
              available,
            });
        ...
        console.warn(
          "CourseContext: backend sync failed, using local state:",
          err.message,
        );
      }
    };
      ...
      if (user) {
        ...
        if (mongoId) {
          enrollmentApi2
            .enroll(mongoId)
            .catch((err) =>
              console.warn("Backend enroll failed:", err.message),
            );
        }
      }
    },
  ...
  const updateCourseProgress = useCallback(
    (yearId, courseId, payload) => {
      ...
      setState((prev) => {
        ...
        return applyYearCompletionAndUnlock(nextState, yearId, {
          ...year,
          enrolled,
        });
      });
      ...
      if (user) {
        ...
        enrollmentApi2
          .updateProgress(targetId, { progress, nextItem, sectionsCompleted })
          .catch((err) =>
