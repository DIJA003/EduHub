...
import { Screen, Card, SectionLabel, Tag, Pill, ProgressBar, Divider, ErrorBox, Avatar, Btn, safeArray, C } from "../components/UI";
          ...
          <View style={{ flex: 1 }}>
            {editMode ? (
              <TextInput value={form.name} onChangeText={(v) => setForm((p) => ({ ...p, name: v }))}
                style={{ fontWeight: "700", fontSize: 16, borderWidth: 1, borderColor: "#E2E8F0", borderRadius: 10, padding: 8, color: "#0F172A" }} />
            ) : (
          ...
          <View key={key} style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            ...
            {editMode && !readOnly ? (
              <TextInput value={form[key]} onChangeText={(v) => setForm((p) => ({ ...p, [key]: v }))}
                style={{ flex: 1, marginLeft: 8, borderWidth: 1, borderColor: "#E2E8F0", borderRadius: 10, padding: 8, fontSize: 13, color: "#0F172A" }}
                autoCapitalize="none" />
            ) : (
